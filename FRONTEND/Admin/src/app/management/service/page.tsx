...
interface ServiceTasksApiResponse {
  success: boolean;
  data: ServiceTask[];
  pagination: {
    currentPage: number;
...
export default function ManageServiceTasks() {
  ...
  const permission = allPermissions?.find(
    (perm) => perm.module === "Manage Service"
  );
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (searchQuery.trim()) {
        url = `/api/admin/search?type=service&query=${encodeURIComponent(searchQuery.trim())}&page=${currentPage}&limit=10`;
      } else {
        url = `/api/admin/tasks/services?page=${currentPage}&limit=10`;
      ...
      const response = await apiCall<ServiceTask[]>(
        url,
        {},
        true
      ) as any;
  ...
  const handleOpenAssignModal = async (task: ServiceTask, reassign: boolean = false) => {
      ...
      <div className="rounded-[10px] bg-white px-5 pb-3 pt-5 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <div className="mb-6 flex items-center justify-between">
          ...
          {permission?.can_create && (
            <button
              onClick={() => router.push("/management/service/add")}
                ...
                >
                  ...
                  <TableCell className="!text-center">
                    <span className={`inline-block rounded-full px-3 py-1 text-sm font-medium ${
                      ...
                      task.task_status === "in_progress" ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-200" :
                  ...
                  <TableCell className="!text-center">
                    <div className="flex items-center justify-center gap-x-3">
                      {permission?.can_view && (
                        <button
                          onClick={() => router.push(`/management/service/${task.task_id}`)}
