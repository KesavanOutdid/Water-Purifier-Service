...
export function LocalDistributorsCard() {
  ...
  const localDistributorKey = `${timeFrame}_top_5_local_distributors` as const;
  const localDistributors = data?.[localDistributorKey] || [];
    ...
    <div className="h-[400px] flex flex-col rounded-[10px] bg-white shadow-1 dark:bg-gray-dark">
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full">
          <thead>
            <tr className="bg-purple-100 dark:bg-purple-900">
              <th className="px-4 py-3 text-left">
                <span className="text-sm text-black-100 dark:text-purple-100">
                  Name
                </span>
              </th>
          ...
          <tbody>
            {localDistributors.map((distributor) => {
              const completionRate =
                distributor.total_tasks > 0
                  ? Math.round(
                      (distributor.completed_tasks / distributor.total_tasks) *
                        100
                    )
                  : 0;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="text-sm text-dark dark:text-white">
                      {distributor.total_tasks}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className="inline-block rounded-full bg-purple-100 px-2 py-1 text-xs text-purple-700 dark:bg-purple-900 dark:text-purple-200">
                      {distributor.completed_tasks}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
